...
set.seed(54)
EMR_long_diags_patient<-EMR_diags_demo[match(unique(EMR_diags_demo$Patient_index),EMR_diags_demo$Patient_index)
...
for( i in 1:10){
  ...
  id_test_data<-unlist(lapply(EMR_test_data$Patient_index, function(x) grep(x,EMR_diags_demo$Patient_index)))
  EMR_long_diags_test<-EMR_diags_demo[id_test_data,] 
  EMR_long_diags_train<-EMR_diags_demo[-(id_test_data),] 
