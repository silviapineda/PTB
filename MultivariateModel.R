...
load("Data/EMR_long_diags_multi_all.Rdata")
load("Data/EMR_long_meds_multi_all.Rdata")
demographics<-read.csv("Data/EMR_patients_patient_race.csv")

###Numbers for each specific dataset
table(EMR_long_diags_multivariate[!duplicated(EMR_long_diags_multivariate$Patient_index),"Term"])
table(EMR_long_labs_multivariate[!duplicated(EMR_long_labs_multivariate$Patient_index),"Term"])
table(EMR_long_meds_multivariate[!duplicated(EMR_long_meds_multivariate$Patient_index),"Term"])
...
EMR_labs_diags<-merge(EMR_long_labs_multivariate,EMR_long_diags_multivariate,by="ID") #13,159
EMR_labs_diags_meds<-merge(EMR_labs_diags,EMR_long_meds_multivariate,by="ID") #2,132 (1877 Term, 255 PTB)
EMR_labs_diags_meds_demo<-merge(EMR_labs_diags_meds,demographics,by="Patient_index")
EMR_labs_diags_meds_demo<-EMR_labs_diags_meds_demo[!duplicated(EMR_labs_diags_meds_demo$ID),] #2131 (1876 Term, 255 PTB)
colnames(EMR_labs_diags_meds_demo)[3]<-"Term"
EMR_labs_diags_meds_demo<-EMR_labs_diags_meds_demo[,-4]
EMR_labs_diags_meds_demo<-EMR_labs_diags_meds_demo[,-c(254:255,728:729,838:839)] ##2131 samples, 835 variables

table(EMR_labs_diags_meds_demo[!duplicated(EMR_labs_diags_meds_demo$Patient_index),"Term"])

###Filter by number of samples with info
tab_term<-NULL
tab_ptb<-NULL
for(i in 4:ncol(EMR_labs_diags_meds_demo)){
  tab_term[i]<-table(EMR_labs_diags_meds_demo[which(EMR_labs_diags_meds_demo$Term=="0"),i])[1]
  tab_ptb[i]<-table(EMR_labs_diags_meds_demo[which(EMR_labs_diags_meds_demo$Term=="1"),i])[1]
}
tab_ptb[1:3]<-255
tab_term[1:3]<-1876

EMR_labs_diags_meds_demo_Term<-EMR_labs_diags_meds_demo[which(EMR_labs_diags_meds_demo$Term=="0"),which(tab_term<1873)] #At least four samples
EMR_labs_diags_meds_demo_PTB<-EMR_labs_diags_meds_demo[which(EMR_labs_diags_meds_demo$Term=="1"),which(tab_ptb<252)] #At least four samples
id.match<-match(colnames(EMR_labs_diags_meds_demo_PTB),colnames(EMR_labs_diags_meds_demo_Term))
PTB_data<-EMR_labs_diags_meds_demo_PTB[,which(is.na(id.match)==F)]
Term_data<-EMR_labs_diags_meds_demo_Term[,na.omit(id.match)]
EMR_labs_diags_meds_demo_filter<-rbind(PTB_data,Term_data)
EMR_labs_diags_meds_demo_filter<-cbind(EMR_labs_diags_meds_demo[,1:3],EMR_labs_diags_meds_demo_filter) #426

save(EMR_labs_diags_meds_demo_filter,file="Data/EMR_multi_all.Rdata")

###For the downsampling 
EMR_labs_diags_meds_demo_Term<-EMR_labs_diags_meds_demo[which(EMR_labs_diags_meds_demo$Term=="0"),which(tab_term<1871)] #At least six samples
EMR_labs_diags_meds_demo_PTB<-EMR_labs_diags_meds_demo[which(EMR_labs_diags_meds_demo$Term=="1"),which(tab_ptb<250)] #At least six samples
id.match<-match(colnames(EMR_labs_diags_meds_demo_PTB),colnames(EMR_labs_diags_meds_demo_Term))
PTB_data<-EMR_labs_diags_meds_demo_PTB[,which(is.na(id.match)==F)]
Term_data<-EMR_labs_diags_meds_demo_Term[,na.omit(id.match)]
EMR_labs_diags_meds_demo_filter<-rbind(PTB_data,Term_data)
EMR_labs_diags_meds_demo_filter<-cbind(EMR_labs_diags_meds_demo[,1:3],EMR_labs_diags_meds_demo_filter) #323

save(EMR_labs_diags_meds_demo_filter,file="Data/EMR_multi_all_down.Rdata")

#########################
##  Run in the server  ##
library(glmmLasso)
load("Data/EMR_multi_all.Rdata")
table(EMR_labs_diags_meds_demo_filter[!duplicated(EMR_labs_diags_meds_demo_filter$Patient_index),"Term"])
table(EMR_labs_diags_meds_demo_filter[,"Term"])
...
set.seed(54)
EMR_long_patient<-EMR_labs_diags_meds_demo_filter[match(unique(EMR_labs_diags_meds_demo_filter$Patient_index),EMR_labs_diags_meds_demo_filter$Patient_index)
                                                  ,c("Term","Patient_index")] #1566 unique patients
table(EMR_long_patient[,"Term"]) #1409 Term and 157 PTB
...
for( i in 1:10){
  ...
  id_test_data<-unlist(lapply(EMR_test_data$Patient_index, function(x) grep(x,EMR_labs_diags_meds_demo_filter$Patient_index)))
  EMR_long_test<-EMR_labs_diags_meds_demo_filter[id_test_data,] 
  EMR_long_train<-EMR_labs_diags_meds_demo_filter[-(id_test_data),] 
  ...
  for(j in 1:length(lambda)){
    ...
    glm1 <- try(glmmLasso(Term ~ X.Int.l.Normaliz.Ratio._order+Activated.Partial.Thromboplastin.Time_order+AFI..cm._order+Alanine.transaminase_order+Albumin..Random_order+Albumin..Serum...Plasma_order+Alkaline.Phosphatase_order+Anion.Gap_order+Aspartate.transaminase_order+Atrial.Rate_order+Basophil.Abs.Count_order+Bile.Acids..Total_order+Bilirubin..Total_order+Biophysical.Profile.Score..of.10._order+C.Reactive.Protein_order+Calcium..total..Serum...Plasma_order+Calculated.P.Axis_order+Calculated.R.Axis_order+Calculated.T.Axis_order+Carbon.Dioxide..Total_order+Chloride..Serum...Plasma_order+Complement.C3..serum_order+Creat.per.Day..UR_order+Creatinine_order+Creatinine..random.urine_order+DEEPEST.VERTICAL.POCKET..CM._order+eGFR.if.African.Amer_order+eGFR.if.non.African.American_order+Eosinophil.Abs.Ct_order+Fetal.Breathing_order+Fetal.Movement_order+Fetal.Tone_order+FHR.Baseline_order+Fibrinogen..Functional_order+Fluid.Volume_order+Free.T4_order+Gamma.Glutamyl.Transpeptidase_order+Glucose.Loading.Screen_order+Glucose..120.Mins_order+Glucose..60.Mins_order+Glucose..fasting_order+Glucose..Fasting..Pregnant..2.hr.GTT_order+Glucose..meter.download_order+Glucose..non.fasting_order+Hematocrit_order+Hemoglobin_order+Hemoglobin.A_order+Hemoglobin.A1c_order+Hemoglobin.A2_order+Hours.Collected_order+Imm.Gran..Left.Shift_order+Lactate.Dehydrogenase..Serum...Plasma_order+Lipase_order+Lymphocyte.Abs.Cnt_order+Magnesium..Serum...Plasma_order+MCH_order+MCHC_order+MCV_order+Monocyte.Abs.Count_order+Neutrophil.Absolute.Count_order+Nonstress.Test_order+OGT.Glucola.Dose_order+P.R.Interval_order+pH..UA_order+Phosphorus..Serum...Plasma_order+Platelet.Count_order+POCT.pH..UA_order+POCT.Spec.Grav..UA_order+Potassium..Serum...Plasma_order+Prot.Concentration.UR_order+Prot.Total.per.Day.UR_order+Protein.Creat.Ratio..random_order+Protein..Total..Serum...Plasma_order+Protein..UA_order+PT_order+QRS.Duration_order+QT.Interval_order+QTcb_order+RBC.Count_order+RBCs...Units.Ready_order+Sodium..Serum...Plasma_order+Specific.Gravity_order+Tacrolimus_order+Thyroid.Stimulating.Hormone_order+Total.Volume.Collected_order+Urea.Nitrogen..Serum...Plasma_order+Ventricular.Rate_order+Vitamin.D..25.Hydroxy_order+WBC.Count_order+Weight.In.Kg_order+X.Int.l.Normaliz.Ratio.+Activated.Partial.Thromboplastin.Time+AFI..cm.+Alanine.transaminase+Albumin..Random+Albumin..Serum...Plasma+Alkaline.Phosphatase+
                          ...
                          ,rnd = list(Patient_index=~1),family = family, data = EMR_long_train, lambda=lambda[j]),silent=TRUE)
  ...
  predictions[[i]] <- predict(glm_final, EMR_long_test, type="response",s=lambda[opt])
  original[[i]]<-EMR_long_test$Term
}
save(predictions,original,file="Results/predictions_multivariate_ALL.Rdata")

######################
## Analysis Results ##
######################
load("Results/predictions_multivariate_ALL.Rdata")
library(AUC)
auc<-NULL
